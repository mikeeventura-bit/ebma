...
| **Black Radish in the main menu** | Currently **absent by request**, to avoid the shop-or-donate ambiguity a bare menu item creates. It is reachable from its homepage feature section. This sits against §05 ("dedicated flagship initiative destination") and §07 ("major visual presence"), so revisit before launch: if Black Radish stays out of the nav, the homepage section has to carry all of its discovery. | Nav, `/black-radish` |
